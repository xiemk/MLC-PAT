...
from src_files.utils.helper import function_mAP, get_raw_dict, clean_state_dict
...
def get_args():
    ...
    args.output = os.path.join(args.output, args.data_name, f'inference_{args.model_name}_{args.pretrain_type}_{args.image_size}_{args.batch_size}')
    return args
...
def main_worker(args, logger):
    ...
    probs, preds, labels = validate(val_loader, model, args, logger)
...
def validate(val_loader, model, args, logger):
    ...
    outputs = torch.cat(preds).numpy()
    labels = torch.cat(labels).numpy()

    # calculate mAP
    mAP, APs= function_mAP(labels, outputs)
    print("Calculating mAP:")  

    logger.info("#####################################################################")

    logger.info("mAP ori {:.2f}".format(mAP))
    # logger.info(np.round(APs, decimals=1))
    ##########################################  Test: OF1 CF1  ###################################################
    sorted_outputs = -np.sort(-outputs, axis=0)

    n_lb = len(outputs)

    # pos_ratio = labels.sum(0)/labels.shape[0]
    global pos_ratio

    # print(pos_ratio)

    indices = [int(x)-1 for x in pos_ratio*n_lb]
    ...
    logger.info("CP {:.2f}, CR {:.2f}, CF1 {:.2f}, OP {:.2f}, OR {:.2f}, OF1 {:.2f}".format(CP*100, CR*100, CF1*100, OP*100, OR*100, OF1*100))

    logger.info("#####################################################################")

    ##############################################################################################################
    
    return outputs, pseudo_labels, labels
